...
class GridsCanvas extends Canvas {
	...
	public void paint(Graphics g) {
		...
		for(int i = 0; i < points.size(); i++) {
			...
			g.fillOval(points.get(i).getX() - CircleSize / 2, points.get(i).getY() - CircleSize / 2, CircleSize, CircleSize);
...
public class GUI extends Frame {
	public GUI(List<MyPoint> points) {
    	...
    	GridsCanvas xyz = new GridsCanvas(GridsCanvas.GridSize, GridsCanvas.GridSize, points);
    	this.add(xyz);
